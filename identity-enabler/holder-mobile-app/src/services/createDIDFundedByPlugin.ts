...
import { NODE_ENDPOINT, PLUGIN_ENDPOINT, TOKEN } from "./env";
import type { Key } from "../models/types/key";

export async function createDID(): Promise<{ doc: FullDoc, keys: Key[] }> {
    ...
    const { publicKeys, bech32Addresses, privateKeys } = await generateAddresses(NODE_ENDPOINT, TOKEN, 2);
    const keys: Key[] = [];
    for (let i = 0; i < publicKeys.length; i++) {
        const key: Key = {
            publicKey: publicKeys[i],
            privateKey: privateKeys[i],
            bech32Address: bech32Addresses[i]
        };
